...
/*********************************************************************
 * _convolveImageHoriz - NOW USES GPU!
 */
  ...
  _KLT_FloatImage imgout)
{
  ...
  if (gpu_enabled) {
    printf("🚀 GPU HORIZONTAL CONVOLUTION (Size: %dx%d, Kernel: %d)\n", 
           imgin->ncols, imgin->nrows, kernel.width);
    
    /* Call GPU function with error handling */
    gpu_convolveImageHoriz(imgin, kernel, imgout);
    
    /* Check if GPU failed and fall back to CPU */
    if (gpu_enabled == 0) {
      printf("⚠️  GPU failed, falling back to CPU\n");
...
/*********************************************************************
 * _convolveImageVert - NOW USES GPU!
 */
  ...
  _KLT_FloatImage imgout)
{
  ...
  if (gpu_enabled) {
    ...
    /* Call GPU function with error handling */
    gpu_convolveImageVert(imgin, kernel, imgout);
